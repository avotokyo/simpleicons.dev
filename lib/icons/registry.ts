...
/** 内存注册表结构。 bySlug: 官方 slug → 元数据 aliasToSlug: 别名（含规范化后的）→ 官方 slug allSlugs: 排序后的全部 slug 列表 */
type Registry = {
...
/** 将用户输入的别名规范化为查找键。 规则：转小写并移除非字母数字字符（如 "Node.js" → "nodedotjs"）。 */
function normalizeAlias(alias: string): string {
  return alias.toLowerCase().replace(/[^a-z0-9]/g, "");
}

/** 从 icons.json 构建内存注册表。 索引 slug、title、old/aka/loc 别名，支持多种方式查找图标。 */
...
/** 将用户输入解析为官方 slug。 依次尝试：精确 slug → 别名表 → 规范化别名。 */
